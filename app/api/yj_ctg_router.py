...
from app.services.yj_service import DashBoardView, USerListView, USerViewTotal, CategorySelect

router = APIRouter(prefix="/ctg", tags=["ctg"])



def get_ctglist_view(conn: Connection = Depends(get_conn)) -> CategorySelect:
    return CategorySelect(conn)


@router.get("/ctg_list")
def ctg_list(
        svc: CategorySelect = Depends(get_ctglist_view)):
    return svc.category_select()
