...
from app.services.yj_service import DashBoardView, USerListView, USerViewTotal, CategorySelect, EventStatus

router = APIRouter(prefix="/event", tags=["event"])



def get_event_status(conn: Connection = Depends(get_conn)) -> EventStatus:
    return EventStatus(conn)


@router.get("/status")
def status(
        svc: EventStatus = Depends(get_event_status)):
    return svc.event_status()
