...
from app.services.yj_service import DashBoardView, ArtistList

router = APIRouter(prefix="/artist", tags=["artist"])



def get_artist_list(conn: Connection = Depends(get_conn)) -> ArtistList:
    return ArtistList(conn)


@router.get("/artist_list")
def artist_list(
        svc: ArtistList = Depends(get_artist_list)):
    return svc.artist_list()
